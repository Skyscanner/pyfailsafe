...
class TestRetryPolicy:
    ...
    def test_should_not_retry_when_exception_is_not_retriable(self):
        retry_policy = ExceptionHandlingPolicy(allowed_retries=3, retriable_exceptions=[BufferError])
    ...
    def test_should_retry_with_more_that_one_exception_type(self):
        retry_policy = ExceptionHandlingPolicy(allowed_retries=3, retriable_exceptions=[BufferError, ValueError])
    ...
    def test_should_raise(self):
        raise_policy = ExceptionHandlingPolicy(raisable_exceptions=[AttributeError])
        assert raise_policy.should_raise(BufferError()) is False
        assert raise_policy.should_raise(ValueError()) is False
        assert raise_policy.should_raise(AttributeError()) is True
